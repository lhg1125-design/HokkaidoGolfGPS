...
# Replace the V1.6.2 name-only dialog with player-count + generated-slot setup.
...
helpers=r'''        private int playerCount(){
        ...
        private String playerName(int i){
            int cnt=playerCount();if(cnt>0 && i>=cnt)return "";
...
# Player strip: create only the selected number of slots.
...
# Score input: 1~4 player cards, centered in the available area.
...
score_input=r'''        private void scoreInput(Canvas c){
            ...
            text(c,ko[selected]+" / H"+hole+" / PAR "+par+" · "+n+"명",m,h*.112f,14,Color.rgb(218,242,222),true);
            ...
            for(int pl=0;pl<n;pl++){
                ...
                text(c,"타수",card.left+115,card.top+78,13,Color.GRAY,true);goldText(c,""+st,card.left+226,card.top+92,34f,INK);
                text(c,"퍼트",card.left+420,card.top+78,13,Color.GRAY,true);goldText(c,""+pu,card.left+535,card.top+92,34f,INK);
                inputStrokeMinus[pl].set(card.left+96,card.bottom-58,card.left+164,card.bottom-10);inputStrokePlus[pl].set(card.left+282,card.bottom-58,card.left+350,card.bottom-10);
                inputPuttMinus[pl].set(card.left+408,card.bottom-58,card.left+476,card.bottom-10);inputPuttPlus[pl].set(card.right-92,card.bottom-58,card.right-24,card.bottom-10);
...
# Scorecard: keep the proven layout, but draw/total only active players and remove unused summary slots.
sc_start=s.find('        private void score(Canvas c){')
sc_end=s.find('        private void saveRef(',sc_start)
if sc_start<0 or sc_end<0:
    raise SystemExit('v1.6.3 scorecard region not found')
block=s[sc_start:sc_end]
block=block.replace('float w=getWidth(),h=getHeight(),m=w*.045f;','float w=getWidth(),h=getHeight(),m=w*.045f;int n=Math.max(1,playerCount());',1)
block=block.replace('for(int pl=0;pl<4;pl++)','for(int pl=0;pl<n;pl)')
block=block.replace('cardGap*3)/4f','cardGap*(n-1))/n')
block=block.replace('text(c,ko[selected]+" / "+variants[selected][variant],m,h*.112f,14,Color.rgb(218,242,222),true);','text(c,ko[selected]+" / "+variants[selected][variant]+" · "+n+"명",m,h*.112f,14,Color.rgb(218,242,222),true);')
s=s[:sc_start]+block+s[sc_end:]

# Score-input touch loop follows configured player count.
touch_start=s.find('            if(screen==2){')
touch_end=s.find('            if(mapTab.contains(x,y))',touch_start)
if touch_start<0 or touch_end<0:
    raise SystemExit('v1.6.3 touch region not found')
touch=s[touch_start:touch_end]
touch=touch.replace('for(int pl=0;pl<4;pl++)','for(int pl=0;pl<Math.max(1,playerCount());pl++)')
