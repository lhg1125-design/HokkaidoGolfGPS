...
# Generic design renderer. Course art is NEVER regenerated here: the active
# external/source yardage bitmap is aspect-fit into the approved viewport.
# Royal Links is enabled first for field validation; the renderer itself is
# course-agnostic and can be enabled for any subsequent source-backed course.
anchor='        private boolean furanoKing123V1152(){'
pos=s.find(anchor)
if pos<0:
    # V1.15.6 field workflow intentionally stops at V1.15.1, so insert before
    # the common cover helper when Furano-specific patches are absent.
...
helpers=r'''        // V1.15.6 MASTER DESIGN RENDERER
        ...
        private boolean masterRendererV1156(){
            // Royal Links first for today's field test. Expanding to another course is
            // only a source/data registration change; no image-design pipeline is needed.
            return screen==1 && selected==4 && hole>=1 && hole<=18;
        }
        ...
        private Distances masterDistancesV1156(){
            GeoRef g=greenCenterRef(hole);Distances ds=distances(g);
            int center=masterCenterM1156();
            if(ds.center<0){ds.center=center;ds.front=Math.max(0,center-12);ds.back=center+12;}
            return ds;
        }
        ...
        private void drawMasterRendererV1156(Canvas c){
            ...
            // Wood board: fixed design, live data only.
            ...
            Distances ds=masterDistancesV1156();
            int center=ds.center>=0?ds.center:masterCenterM1156();int front=ds.front>=0?ds.front:Math.max(0,center-12);int back=ds.back>=0?ds.back:center+12;
            ...
            c.restore();
            // Raw external yardage source: contain only, never crop/warp/stylize.
            ...
            // Target + standard four-tab nav from the master grammar.
...
if 'if(masterRendererV1156())' not in s:
    s=s.replace(round_sig,round_sig+'\n            if(masterRendererV1156()){drawMasterRendererV1156(c);return;}',1)

# The helpers reused from the Furano patch are small data/UI helpers, not image generation.
# Add equivalents if V1.15.2 was intentionally omitted in this field workflow.
...
if 'V1.15.6 · MASTER SOURCE MAPPER' not in s:
    s=s.replace('V1.15.1 · REFERENCE POLISH','V1.15.1 · REFERENCE POLISH / V1.15.6 · MASTER SOURCE MAPPER',1)
