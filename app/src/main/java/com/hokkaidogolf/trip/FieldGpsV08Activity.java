...
public class FieldGpsV08Activity extends Activity implements LocationListener {
    private static final int REQ = 1808;
    ...
    private void startGps() {
        ...
        try { lm.requestLocationUpdates(LocationManager.GPS_PROVIDER, 800, 0.7f, this); } catch (Exception ignored) {}
    }
    ...
    static final class GolfView extends View {
        ...
        private final SharedPreferences calPrefs;
        ...
        private final int[][][] pars={
                {{5,4,3,4,4,4,3,4,5,4,3,4,3,4,5,4,4,5},{5,5,4,3,4,4,4,3,4,4,4,3,5,4,3,5,4,5}},
                {{5,4,3,4,5,4,4,3,4,4,3,4,4,4,5,3,4,5},{4,3,4,5,4,4,3,4,5,4,4,5,4,3,4,3,4,5}},
                {{4,5,4,3,4,5,4,3,4,4,3,4,5,4,3,4,4,5},{4,5,4,3,4,5,4,3,4,4,3,4,5,4,3,4,4,5}}
        };
        ...
        private long lastAutoHoleAt=0,lastHoleChange=0,lastTap=0,confirmUntil=0;
        ...
        private final RectF courseRect=new RectF(),greenSave=new RectF(),teeSave=new RectF(),gpsSettings=new RectF(),autoBtn=new RectF();
        ...
        private final int BG=Color.rgb(249,250,240),INK=Color.rgb(34,55,40),GREEN=Color.rgb(24,111,68),DEEP=Color.rgb(8,79,52),
                ...
                CARD=Color.WHITE,SOFT=Color.rgb(241,245,235);

        GolfView(Context c,boolean preview){
            super(c); ctx=c; previewMode=preview;
            scorePrefs=c.getSharedPreferences("score_v08",MODE_PRIVATE);
            calPrefs=c.getSharedPreferences("cal_v08",MODE_PRIVATE);
            p.setTypeface(Typeface.create("sans-serif-rounded",Typeface.NORMAL));
            setKeepScreenOn(true);
            if(previewMode){ selected=0; variant=0; }
        }
        ...
        private void maybeAutoHole(){
            if(!autoHole||location==null||selected<0||savedCount("t")<3)return;
        ...
        private void home(Canvas c){
            ...
            pill(c,new RectF(m,h*.135f,m+w*.61f,h*.177f),Color.rgb(229,244,218),"V0.8 · FIELD READY LITE",GREEN,9.3f);
            mascot(c,w*.84f,h*.105f,28,true); speech(c,w*.58f,h*.155f,"현장 모드 준비!",GREEN);
            text(c,"오늘 어디서 칠까요?",m,h*.222f,18,INK,true);
            text(c,"GPS 품질 · TEE/GREEN 저장 · 자동 홀 보조 · 4인 스코어",m,h*.252f,9.2f,Color.GRAY,false);
            float top=h*.286f,ch=h*.116f,gap=h*.014f;int[] ac={Color.rgb(174,222,92),YELLOW,SKY};
            for(int i=0;i<3;i++){
                float y=top+i*(ch+gap);cards[i].set(m,y,w-m,y+ch);softShadow(c,cards[i],26);box(c,cards[i],selected==i?Color.rgb(238,249,222):CARD,26);
                p.setColor(ac[i]);c.drawCircle(m+28,y+30,11,p);text(c,"DAY "+(i+1),m+50,y+35,9,GREEN,true);text(c,jp[i],m+22,y+70,18,INK,true);text(c,ko[i],m+22,y+94,10,Color.GRAY,false);
                if(location!=null){int dm=(int)Math.round(distanceToCourse(location,i));String ds=dm<10000?dm+"m":"약 "+Math.round(dm/1000f)+"km";pill(c,new RectF(w-m-105,y+17,w-m-14,y+48),SOFT,ds,selected==i?GREEN:Color.GRAY,8.5f);}
            ...
            float vy=h*.692f;text(c,"코스 선택",m,vy,10,Color.GRAY,true);varA.set(m,vy+12,w*.48f,vy+58);varB.set(w*.52f,vy+12,w-m,vy+58);
            pillButton(c,varA,variant==0?GREEN:CARD,selected<0?"A COURSE":variantNames[selected][0],variant==0?Color.WHITE:INK);pillButton(c,varB,variant==1?GREEN:CARD,selected<0?"B COURSE":variantNames[selected][1],variant==1?Color.WHITE:INK);
            ...
            text(c,"모든 거리 m · 오프라인 스코어 저장",w/2,h*.925f,9,Color.GRAY,false,Paint.Align.CENTER);
        }

        private void round(Canvas c){
            float w=getWidth(),h=getHeight(),m=w*.045f;int par=currentPar();int officialM=(int)Math.round(currentYards()*.9144);float pulse=(float)(.5+.5*Math.sin(SystemClock.uptimeMillis()/320.0));
            text(c,ko[selected]+" · "+variantNames[selected][variant],m,h*.043f,11,Color.GRAY,true);text(c,"H"+hole,m,h*.093f,37,INK,true);pill(c,new RectF(m+76,h*.062f,m+145,h*.102f),Color.rgb(229,244,218),"PAR "+par,GREEN,10.5f);
            String gps=location==null?"GPS 찾는 중…":"GPS ±"+Math.round(location.getAccuracy())+"m";int gpsCol=location==null?CORAL:(location.getAccuracy()<=8?GREEN:(location.getAccuracy()<=15?Color.rgb(192,135,30):CORAL));
            pill(c,new RectF(w-m-135,h*.021f,w-m,h*.061f),location==null?Color.rgb(255,238,229):Color.rgb(235,245,229),gps,gpsCol,9.4f);

            GeoRef green=getRef("g",hole);Distances ds=distances(green);
            RectF range=new RectF(m,h*.119f,w-m,h*.226f);gradient(c,range,DEEP,GREEN,28);sheen(c,range,28);
            metric(c,"FRONT",ds.front<0?"--":ds.front+"m",w*.20f,h*.151f);metric(c,"CENTER",ds.center<0?"--":ds.center+"m",w*.50f,h*.151f);metric(c,"BACK",ds.back<0?"--":ds.back+"m",w*.80f,h*.151f);

            int gCount=savedCount("g"),tCount=savedCount("t");String q=gpsQuality();
            pill(c,new RectF(m,h*.234f,w*.30f,h*.265f),gpsQualityBg(),q,gpsQualityColor(),8.2f);
            pill(c,new RectF(w*.315f,h*.234f,w*.70f,h*.265f),CARD,"DB  G "+gCount+"/18 · T "+tCount+"/18",gCount>0?GREEN:Color.GRAY,8.1f);
            autoBtn.set(w*.715f,h*.234f,w-m,h*.265f);pill(c,autoBtn,autoHole?Color.rgb(229,244,218):CARD,autoHole?"AUTO ON":"AUTO OFF",autoHole?GREEN:Color.GRAY,8.1f);

            courseRect.set(m,h*.278f,w-m,h*.592f);drawCourse(c,courseRect,par,officialM,green,ds,pulse);
            text(c,"V0.8 · GPS 실거리 / Front·Back은 Center ±12m 추정 · 지도 터치는 공략거리 추정",w/2,h*.615f,8.0f,Color.GRAY,false,Paint.Align.CENTER);

            greenSave.set(m,h*.635f,w*.44f,h*.688f);teeSave.set(w*.46f,h*.635f,w*.72f,h*.688f);gpsSettings.set(w*.74f,h*.635f,w-m,h*.688f);
            ...
            pillButton(c,gpsSettings,CARD,"GPS",INK);

            drawPlayerTabs(c,h*.715f);RectF panel=new RectF(m,h*.758f,w-m,h*.862f);softShadow(c,panel,26);box(c,panel,CARD,26);int stroke=getStroke(player,hole,par),putt=getPutt(player,hole);
            text(c,"타수",m+18,h*.792f,10,Color.GRAY,true);text(c,""+stroke,w*.28f,h*.84f,30,INK,true,Paint.Align.CENTER);minus.set(m+10,h*.8f,m+52,h*.852f);plus.set(w*.38f,h*.8f,w*.47f,h*.852f);roundButton(c,minus,"−",SOFT,Color.GRAY);roundButton(c,plus,"+",Color.rgb(229,244,218),GREEN);
            text(c,"퍼트",w*.56f,h*.792f,10,Color.GRAY,true);text(c,""+putt,w*.70f,h*.84f,29,INK,true,Paint.Align.CENTER);pm.set(w*.53f,h*.8f,w*.61f,h*.852f);pp.set(w*.82f,h*.8f,w*.90f,h*.852f);roundButton(c,pm,"−",SOFT,Color.GRAY);roundButton(c,pp,"+",Color.rgb(226,245,250),SKY);
        ...
        private void drawCourse(Canvas c,RectF r,int par,int officialM,GeoRef green,Distances ds,float pulse){
            long now=SystemClock.uptimeMillis();float phase=(now%2400)/2400f,slide=holeSlideOffset(now);c.save();c.translate(slide,0);gradient(c,r,Color.rgb(232,248,214),Color.rgb(202,236,191),32);
            drawMountains(c,r);drawCloud(c,r.left+r.width()*.16f+phase*10,r.top+32,18);drawCloud(c,r.left+r.width()*.76f-phase*8,r.top+44,14);
            float cx=r.centerX();Path f=new Path();f.moveTo(cx-22,r.bottom-25);if(par==3)f.cubicTo(cx-15,r.centerY()+35,cx+10,r.centerY()-35,cx-8,r.top+60);else if(par==4)f.cubicTo(r.left+r.width()*.38f,r.bottom-r.height()*.28f,r.left+r.width()*.64f,r.top+r.height()*.48f,cx-8,r.top+58);else f.cubicTo(r.left+r.width()*.31f,r.bottom-r.height()*.2f,r.left+r.width()*.69f,r.top+r.height()*.54f,cx-8,r.top+58);f.lineTo(cx+48,r.top+66);f.cubicTo(r.left+r.width()*.72f,r.top+r.height()*.50f,r.left+r.width()*.5f,r.bottom-r.height()*.22f,cx+24,r.bottom-25);f.close();p.setColor(Color.rgb(90,180,87));c.drawPath(f,p);stripes(c,f,r,phase);
            RectF water=new RectF(r.left+16,r.top+r.height()*.50f,r.left+r.width()*.38f,r.bottom-18);p.setColor(BLUE);c.drawOval(water,p);ripples(c,water,phase);p.setColor(YELLOW);c.drawOval(new RectF(cx+38,r.top+r.height()*.43f,cx+83,r.top+r.height()*.50f),p);
            ...
            if(hasTarget&&r.contains(targetX,targetY)){p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(3);p.setColor(CORAL);c.drawCircle(targetX,targetY,12+7*pulse,p);p.setStyle(Paint.Style.FILL);p.setColor(CORAL);c.drawCircle(targetX,targetY,6,p);float ratio=Math.max(0,Math.min(1,(youY-targetY)/(youY-(flagY+10))));int est=Math.round(officialM*ratio);RectF b=new RectF(targetX-62,targetY-48,targetX+62,targetY-17);box(c,b,Color.argb(238,255,255,255),15);text(c,"공략 약 "+est+"m",b.centerX(),b.centerY()+4,8.5f,CORAL,true,Paint.Align.CENTER);}
            String bubble=green==null?"GREEN 좌표 필요!":(ds.center>=0?"핀 중앙 "+ds.center+"m":"GPS 기다리는 중!");mascot(c,r.right-45,r.top+48,22,true);speech(c,r.right-188,r.top+18,bubble,green==null?CORAL:DEEP);
            if(previewMode)pill(c,new RectF(r.left+15,r.bottom-45,r.left+119,r.bottom-16),Color.argb(225,255,255,255),"PREVIEW GPS",CORAL,7.3f);c.restore();
        }

        private void requestSave(int kind){
            ...
            String type=kind==1?"g":"t";String k=refKey(type,hole);calPrefs.edit().putLong(k+"_lat",Double.doubleToRawLongBits(location.getLatitude())).putLong(k+"_lon",Double.doubleToRawLongBits(location.getLongitude())).apply();confirmKind=0;confirmUntil=0;showToast("H"+hole+" "+(kind==1?"GREEN CENTER":"TEE")+" 저장 완료");maybeAutoHole();invalidate();
        }

        private GeoRef getRef(String type,int h){
            String k=refKey(type,h);if(calPrefs.contains(k+"_lat"))return new GeoRef(Double.longBitsToDouble(calPrefs.getLong(k+"_lat",0)),Double.longBitsToDouble(calPrefs.getLong(k+"_lon",0)),false);
            if(previewMode&&selected==0&&variant==0&&h==1){if(type.equals("g"))return new GeoRef(43.25982,143.22836,true);if(type.equals("t"))return new GeoRef(43.25720,143.22836,true);}return null;
        }
        ...
        private Distances distances(GeoRef ref){if(ref==null||location==null)return new Distances(-1,-1,-1);int c=Math.round(distance(location,ref.lat,ref.lon));return new Distances(Math.max(0,c-12),c,c+12);}
        ...
        private String gpsQuality(){if(location==null)return "GPS WAIT";float a=location.getAccuracy();return a<=8?"GPS GOOD":(a<=15?"GPS FAIR":"GPS WEAK");}
        private int gpsQualityColor(){if(location==null)return CORAL;float a=location.getAccuracy();return a<=8?GREEN:(a<=15?Color.rgb(180,125,25):CORAL);}
        private int gpsQualityBg(){if(location==null)return Color.rgb(255,238,229);float a=location.getAccuracy();return a<=8?Color.rgb(229,244,218):(a<=15?Color.rgb(255,246,218):Color.rgb(255,238,229));}

        private void score(Canvas c){
            float w=getWidth(),h=getHeight(),m=w*.05f;text(c,"스코어카드",m,h*.068f,29,INK,true);text(c,ko[selected]+" · "+variantNames[selected][variant],m,h*.10f,10,Color.GRAY,false);mascot(c,w*.88f,h*.078f,20,true);text(c,"4명 한 번에 · 계산은 자동",m,h*.132f,10,Color.GRAY,false);float y=h*.18f;int[] totals={0,0,0,0};
        ...
        private int nearestCourse(Location l){int best=-1;float bd=Float.MAX_VALUE;for(int i=0;i<3;i++){float d=(float)distanceToCourse(l,i);if(d<bd){bd=d;best=i;}}return best;}private double distanceToCourse(Location l,int i){if(l==null)return -1;float[] o=new float[1];Location.distanceBetween(l.getLatitude(),l.getLongitude(),courseLat[i],courseLon[i],o);return o[0];}
        ...
        private void drawToast(Canvas c){if(toastAt==0)return;long age=SystemClock.uptimeMillis()-toastAt;if(age>1800)return;float w=getWidth(),h=getHeight();int a=age<1400?245:(int)(245*(1-(age-1400)/400f));RectF r=new RectF(w*.18f,h*.49f,w*.82f,h*.545f);box(c,r,Color.argb(Math.max(0,a),20,70,45),24);text(c,toastText,w/2,r.centerY()+5,9.7f,Color.WHITE,true,Paint.Align.CENTER);}
        private void drawTapBurst(Canvas c,float h){long age=SystemClock.uptimeMillis()-lastTap;if(age>600||lastDelta==0)return;float t=age/600f;float x=lastDelta>0?getWidth()*.43f:getWidth()*.12f,y=h*.82f-t*45;int a=(int)(255*(1-t));p.setColor(Color.argb(Math.min(220,a),255,255,255));c.drawCircle(x,y,18,p);text(c,lastDelta>0?"+1":"−1",x,y+5,10,Color.argb(a,24,111,68),true,Paint.Align.CENTER);}
        ...
        private void speech(Canvas c,float x,float y,String s,int col){RectF b=new RectF(x,y,x+138,y+36);softShadow(c,b,18);box(c,b,Color.argb(242,255,255,255),18);text(c,s,b.centerX(),b.centerY()+4,8.7f,col,true,Paint.Align.CENTER);}
        ...
        @Override public boolean onTouchEvent(MotionEvent e){
            ...
            if(screen==1&&courseRect.contains(x,y)){hasTarget=true;targetX=x;targetY=y;invalidate();return true;}
            if(autoBtn.contains(x,y)){autoHole=!autoHole;showToast(autoHole?"AUTO HOLE ON":"AUTO HOLE OFF");invalidate();return true;}if(greenSave.contains(x,y)){requestSave(1);return true;}if(teeSave.contains(x,y)){requestSave(2);return true;}if(gpsSettings.contains(x,y)){launchGpsSettings();return true;}
            for(int i=0;i<4;i++)if(playerTabs[i].contains(x,y)){player=i;invalidate();return true;}
            int par=currentPar();if(minus.contains(x,y)){setStroke(player,hole,Math.max(1,getStroke(player,hole,par)-1));lastTap=SystemClock.uptimeMillis();lastDelta=-1;}else if(plus.contains(x,y)){setStroke(player,hole,getStroke(player,hole,par)+1);lastTap=SystemClock.uptimeMillis();lastDelta=1;}else if(pm.contains(x,y)){setPutt(player,hole,Math.max(0,getPutt(player,hole)-1));lastTap=SystemClock.uptimeMillis();lastDelta=-1;}else if(pp.contains(x,y)){setPutt(player,hole,getPutt(player,hole)+1);lastTap=SystemClock.uptimeMillis();lastDelta=1;}else if(prev.contains(x,y)){if(hole>1){hole--;holeDirection=-1;lastHoleChange=SystemClock.uptimeMillis();hasTarget=false;}}else if(next.contains(x,y)){if(hole<18){hole++;holeDirection=1;lastHoleChange=SystemClock.uptimeMillis();hasTarget=false;}}else if(mapTab.contains(x,y)){screen=1;}else if(scoreTab.contains(x,y)){screen=2;}invalidate();return true;
        }
