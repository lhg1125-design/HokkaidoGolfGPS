#!/usr/bin/env bash
set -euo pipefail
APK="HokkaidoGolfGPS-v1.10.6-hokkaido-126-hole-pack-debug.apk"
...
if adb logcat -d | grep -E "FATAL EXCEPTION|Process: ${PKG}" | grep -q "${PKG}\|FATAL EXCEPTION"; then
  echo "App crash detected during V1.10.6 preview run"; adb logcat -d | tail -500; exit 1
fi
printf 'V1.10.6 final 126-hole yardage screenshots:\n'; ls -lh "$OUT"/*.png
