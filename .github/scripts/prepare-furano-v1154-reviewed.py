...
# 2) The approved app chrome is created in the base itself (no later chrome redraw).
# 3) Course geometry is never warped/rotated/redrawn. Only alpha-tight crop,
#    high-quality scale and mild display sharpening are allowed.
...
# Alpha-tight crop: removes only transparent/neutral exterior created by the
# approved-source lock. It does not alter any course pixel or geometry.
...
# Approved-screen course slots. H2/H3 are slightly wider/taller than the old
# safe slot, matching the user-confirmed full-screen previews without clipping.
...
# Mild display sharpening after resize only. Geometry/alpha remain identical.
old="cr=ci.resize((round(ww),round(hh)),Image.Resampling.LANCZOS);sh=Image.new('RGBA',cr.size,(0,0,0,0))"
new="cr=ci.resize((round(ww),round(hh)),Image.Resampling.LANCZOS);cr=cr.filter(ImageFilter.UnsharpMask(radius=1.0,percent=115,threshold=2));sh=Image.new('RGBA',cr.size,(0,0,0,0))"
if old in s:
    s=s.replace(old,new,1)
elif 'UnsharpMask(radius=1.0,percent=115,threshold=2)' not in s:
    raise SystemExit('V1.15.5 resize anchor missing')

# Draw the full approved blue header directly into the frozen base. Previous
# builds depended on a later chrome redraw, which caused the approved screen to
...
# keep the blue background from HEADER_SOURCE; draw only stable foreground chrome
...
# simple sunny-cloud weather mark, fixed artwork rather than runtime/system icon
...
p.write_text(s)
print('V1.15.5 NORMALIZE: exact 408 H2/H3 geometry + approved full header + fixed slots + one ruler')
