...
def wood_card(im,d,y,day,jp_name,ko_name,accent):
    box=(100,y,980,y+205);shadow(im,box,34)
    rr(d,box,34,(244,205,140,255),outline=(118,77,34,255),width=5)
    rr(d,(118,y+18,962,y+187),27,(255,225,171,255),outline=(196,139,69,255),width=3)
    for gx in range(145,950,115):
        d.arc((gx,y+43,gx+90,y+163),start=110,end=250,fill=(216,163,91,170),width=2)
    rr(d,(55,y+54,133,y+132),39,accent);centered(d,(94,y+93),str(day),font(34),(255,255,255,255))
    centered(d,(540,y+76),jp_name,font(43,True),(58,43,31,255),stroke=2,stroke_fill=(255,244,215,255))
    centered(d,(540,y+137),ko_name,font(28),(57,75,51,255))

# HOME: the approved reference image is used literally as the hero. Lower UI is raster-rendered with the same rounded font language.
...
cards=[('上士幌ゴルフ場','가미시호로 GC',(77,160,83,255)),('富良野ゴルフコース','후라노 GC',(131,89,180,255)),('サホロカントリークラブ','사호로 CC',(38,128,84,255))]
for i,(jp_name,ko_name,accent) in enumerate(cards):
    wood_card(home,d,[755,1010,1265][i],i+1,jp_name,ko_name,accent)
...
# COURSE: the approved phone-map crop is the visual core; typography is pre-rendered with rounded fonts instead of rough Canvas text.
...
for x,lab in [(205,'FRONT'),(540,'CENTER'),(875,'BACK')]: centered(d,(x,390),lab,font(25),(205,239,214,255))
map_box=(55,600,1025,1605);shadow(co,map_box,42);rr(d,map_box,42,(214,241,199,255))
...
d.text((65,70),'스코어카드',font(50),fill=(255,255,255,255));d.text((65,135),'4명 함께 · 자동 합계',font(25),fill=(211,243,220,255))
rr(d,(55,330,1025,410),22,(7,78,50,255));cols=[(105,'HOLE'),(265,'PAR'),(445,'P1'),(610,'P2'),(775,'P3'),(940,'P4')]
for x,lab in cols:centered(d,(x,370),lab,font(22),(255,255,255,255))
sy=430
for i in range(18):
    y=sy+i*73;rr(d,(55,y,1025,y+62),18,(255,255,255,255) if i%2==0 else (255,253,240,255))
shadow(sc,(55,1805,1025,2115),34);rr(d,(55,1805,1025,2115),34,(10,103,62,255));d.text((90,1855),'ROUND TOTAL',font(28),fill=(210,242,219,255))
