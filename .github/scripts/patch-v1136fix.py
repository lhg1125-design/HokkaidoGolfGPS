...
if 'V1.13.6 · ROUND LOG' not in s:
    raise SystemExit('v1.13.6 fix requires ROUND LOG build')

# V1.13.4/1.13.5 replace drawFieldNav up to fieldReadyLabel. Older V1.12.4
# helpers lived inside that interval, so restore them after every geo rewrite.
if 'private void drawHoleStepButtonsV1124(' not in s:
    anchor='        private String fieldReadyLabelV1114(){'
    idx=s.find(anchor)
    if idx<0: raise SystemExit('fieldReady helper anchor missing')
...
# V1.13.5 replaces the whole saveRef -> getRef interval, which can remove the
# old GREEN wrapper. The visible GREEN CENTER control should call saveRef(1)
# directly; this is the canonical calibrated-center capture path.
s=s.replace('if(greenSave.contains(x,y)){saveGreenPoint();return true;}',
            'if(greenSave.contains(x,y)){saveRef(1);return true;}')

p.write_text(s)
print('restored circular hole-step helpers + GREEN CENTER capture after V1.13.5 geo rewrite')
